...
API_CALL(AccountManager::Api_SetAdmin)
{
    ...
    // avoid admin being able to remove himself or the very last admin
    if (adminCount <=1 || login.toAscii() == account.GetLogin() )
        return ApiManager::ApiError("Can't remove admin privilege for yourself or the last administrator");
